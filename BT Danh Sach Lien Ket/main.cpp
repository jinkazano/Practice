#define _CRT_SECURE_NO_WARNINGS // Dung VS 2015 nen phai them
...
void nhap (LIST &L)
{
    ...
    do
    {
        ...
        NODE *p=CreateNode(x);
        AddLast(L,p);
        printf("\n\n~~~~~NHAN phim bat ki de tiep tuc nhap hoac NHAN 0 de dung nhap");
    } while (1);
}
void output (sv x)
{
...
void xoa (LIST &L)
{
    ...
    while (p!=NULL)
    {
        if(strcmp(a, p->info.ten)==0)    break;
        else printf("\nKo co sv can xoa.");
        q=p;
        p=p->next;
    }
...
void themsau(LIST L) {
	...
	int tuoi;
	float diem;
	printf("\n THEM SINH VIEN VAO PHiA SAU MOT SINH VIEN ÐA CO TEN TRONG DANH SACH");
	printf("\n Nhap ten cua mot sinh vien da co ten trong danh sach : ");
	fflush(stdin);
	gets(ten);
	tim = L.head;
	while ((tim != NULL) && strcmp(tim->ten, ten)) {
		tim = tim->next;
		if (tim == NULL) {
			printf("\n Ten cua sinh vien ban vua nhap vao khOng co trong danh sach ");
		}
		else {
			p = (sv *)malloc(sizeof(sv));
			printf("\n Nhap ten cua sinh vien ban muon bo sung vao danh sach : ");
			fflush(stdin);
			gets(ten);
			printf("\n Nhap tuoi cua sinh vien ban muon bo sung vao danh sach : ");
			scanf("%d", &tuoi);
			p->tuoi = tuoi;
			printf("\n Nhap vao diem trung binh cua sinh vien ban muon bo sung vao danh sach : ");
			scanf("%f", &diem);
